...
(function() {
    ...
    function insertButton() {
        if (buttonInserted) {
            console.log('IINA button already inserted (flag check)');
            return true;
        }

        const existingButton = document.querySelector('[data-iina-button="true"]');
        if (existingButton) {
            console.log('IINA button already exists in DOM');
            buttonInserted = true;
            return true;
        }
        ...
        if (topLevelButtons) {
            ...
            if (iinaButton) {
                ...
                buttonInserted = true;
                console.log('IINA button inserted successfully for video:', getVideoId());
                return true;
            }
        } else {
            console.log('Top level buttons not found');
        }
        return false;
    }

    function init() {
        console.log('Initializing IINA button for URL:', location.href);
        ...
        const interval = setInterval(() => {
            ...
            if (success || attempts >= maxAttempts) {
                ...
                if (!success && attempts >= maxAttempts) {
                    console.log('IINA button insertion failed after', maxAttempts, 'attempts');
                }
            }
        }, 200);
    }
    ...
    document.addEventListener('yt-navigate-finish', function() {
        console.log('YouTube navigation detected via yt-navigate-finish');
        setTimeout(init, 300);
    });
